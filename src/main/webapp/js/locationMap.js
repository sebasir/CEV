...
function setMapCenter(params) {
	...
	if (typeof params.tooltip === "string") {
		...
		for ( var s in params.tooltip) {
			tooltipText = tooltipText + '<h3>   '
					+ params.tooltip[s].scientificName + '</h3>';
			tooltipText = tooltipText + '<br/>';
			tooltipText = tooltipText + '<p>' + params.tooltip[s].commonName
					+ '</p>';
	...
	if (isNumeric(paramLat) && isNumeric(paramLon)) {
		...
		if (latitude != 0 || longitude != 0) {
			setMapLocation({
				lat : latitude,
				lng : longitude
			}, theZoom, paramName, infoWindow);
		} else {
			locateName(new google.maps.Geocoder(), paramName, infoWindow,
					setMapLocation);
		}
	}
}
